...
**OBRIGATÓRIO — Caranguejo Squad (pré-implementação):**

- **@crab DEVE rodar ANTES de qualquer feature, épico, investimento ou decisão significativa**
- `*review {input}` → análise em 9 dimensões → Relatório de Pinça
- Findings CRÍTICOS bloqueiam todos os agentes. Findings ALTOS devem ser endereçados.
- Dimensões: Fundação · Alicerce · Base/Dados · Estrutura · Visual · Segurança · Escala · Tempo · Ecossistema

**When to use specialized agents:**

- Análise de risco (pré-implementação) → Use @crab (PRIMEIRO, sempre)
